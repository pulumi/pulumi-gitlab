 ...
 * 
 * &gt; `READ_ADMIN_*` permissions grant read-only access to the Admin Area and cannot be used on a member role. Use the `gitlab.AdminRole` resource to create a custom admin role instead.
 * 
 * **Upstream API**: [GitLab GraphQL API docs](https://docs.gitlab.com/api/graphql/reference/#mutationmemberrolecreate)
 * 
 * ## Import
 * 
 * Starting in Terraform v1.5.0, you can use an import block to import `gitlab.MemberRole`. For example:
 * 
...
public class MemberRole extends com.pulumi.resources.CustomResource {
    ...
    /**
     * @return The base access level for the custom role. Valid values are: `DEVELOPER`, `GUEST`, `MAINTAINER`, `MINIMAL_ACCESS`, `OWNER`, `REPORTER`, `SECURITY_MANAGER`
     * 
     */
