PROJECT_NAME := GitLab Package
include build/common.mk

PACK             := gitlab
PACKDIR          := sdk
PROJECT          := github.com/pulumi/pulumi-gitlab
NODE_MODULE_NAME := @pulumi/gitlab
...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-gitlab/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${PROVIDER}

generate_schema:: tfgen
	$(TFGEN) schema --out ./provider/cmd/${PROVIDER}

tfgen::
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-gitlab/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${TFGEN}
