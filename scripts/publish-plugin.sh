...
PLUGIN_PACKAGE_NAME="pulumi-resource-gitlab-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
(cd provider && go build \
   -ldflags "-X github.com/pulumi/pulumi-gitlab/provider/v2/pkg/version.Version=${VERSION}" \
